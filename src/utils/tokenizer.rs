...
impl Tokenizer {
    ...
    pub fn encode(&self, text: &str) -> Result<Vec<u32>> {
        ...
        for ch in text.chars() {
            let token = ch.to_string();
            let token_id = self.vocab.get(&token)
                .copied()
    ...
    pub fn decode(&self, token_ids: &[u32]) -> Result<String> {
        ...
        for &token_id in token_ids {
            if let Some(token) = self.reverse_vocab.get(&token_id) {
                ...
                if token == &self.config.bos_token || token == &self.config.eos_token || token == &self.config.pad_token {
                    continue;
                }
                text.push_str(token);
            } else {
    ...
    pub fn pad_token_id(&self) -> Result<u32> {
        self.vocab.get(&self.config.pad_token)
            .copied()
    ...
    pub fn unk_token_id(&self) -> Result<u32> {
        self.vocab.get(&self.config.unk_token)
            .copied()
    ...
    pub fn bos_token_id(&self) -> Result<u32> {
        self.vocab.get(&self.config.bos_token)
            .copied()
    ...
    pub fn eos_token_id(&self) -> Result<u32> {
        self.vocab.get(&self.config.eos_token)
            .copied()
    ...
    pub fn think_start_token_id(&self) -> Result<u32> {
        self.vocab.get(&self.config.think_start_token)
            .copied()
    ...
    pub fn think_end_token_id(&self) -> Result<u32> {
        self.vocab.get(&self.config.think_end_token)
            .copied()

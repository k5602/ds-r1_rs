...
impl EvalMetrics {
    ...
    pub fn update(&mut self, correct: bool, reasoning_length: usize, generation_time_ms: u64) {
        ...
        self.avg_reasoning_length = (prev_avg_length * (self.total_examples - 1) as f32
            + reasoning_length as f32)
            / self.total_examples as f32;

        let prev_avg_time = self.avg_generation_time_ms;
        self.avg_generation_time_ms = (prev_avg_time * (self.total_examples - 1) as f64
...
impl Evaluator {
    ...
    pub fn evaluate(&self, _examples: &[EvalExample]) -> Result<EvalMetrics> {
        ...
        Err(ModelError::Evaluation(
            "Evaluator not implemented yet".to_string(),
        ))
    }

...
use ds_r1_rs::{
    ...
    training::{DifficultyLevel, MathDataset},
};
...
fn test_problem_solving_with_dataset(
    dataset: &MathDataset,
    ...
    let mut solver = MathProblemSolver::new(config.thinking_token_id, config.thinking_token_id + 1);
            ...
            match solver.solve_problem(&problem.problem_text) {
                Ok(solution) => {
                    ...
                    let matches = solution
                        ...
                        .as_ref()
                        .map(|ans| {
                            ans.contains(&problem.expected_answer)
                                || problem.expected_answer.contains(ans)
                        })
                        .unwrap_or(false);
...
fn demonstrate_dataset_serialization(
    dataset: &MathDataset,
) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "Successfully imported {} problems",
        imported_dataset.problems.len()
    );
    ...
    println!(
        "- Match: {}",
        if original_stats.total_problems == imported_stats.total_problems {
            "✅"
        } else {
            "❌"
        }

...
use ds_r1_rs::utils::{
    EvaluationHarness, TestDatasets, EvaluationReportGenerator, ModelInfo,
    ReasoningEvaluator, ReasoningBenchmark, ProblemCategory, DifficultyLevel
};
...
fn demo_evaluation_harness() -> Result<()> {
    ...
    let simple_benchmark = ReasoningBenchmark {
        ...
        problems: vec![
            ...
            ds_r1_rs::utils::evaluation::ReasoningProblem {
                ...
                problem_text: "If all cats are mammals and Fluffy is a cat, is Fluffy a mammal?".to_string(),
    ...
    println!("  - Accuracy: {:.1}%", (results.solved_correctly as f32 / results.total_problems as f32) * 100.0);
    println!("  - Average reasoning quality: {:.2}", results.average_metrics.overall_quality);
    println!("  - Total time: {:.2}s", results.performance_metrics.total_time.as_secs_f32());
    println!();
...
fn demo_test_datasets() -> Result<()> {
    ...
    for dataset in &datasets {
        ...
        println!("     - Easy: {}, Medium: {}, Hard: {}", easy_count, medium_count, hard_count);
        
        // Show sample problem
        if let Some(sample) = dataset.problems.first() {
            println!("     - Sample: {}", sample.problem_text.chars().take(50).collect::<String>());
    ...
    for example in reasoning_examples.iter().take(2) {
        ...
        println!("  Expected steps: {}", example.expected_reasoning_steps.len());
...
fn demo_evaluation_reports() -> Result<()> {
    ...
    println!("  - Model: {} v{}", report.model_info.model_name, report.model_info.version);
    println!("  - Overall Accuracy: {:.1}%", report.overall_summary.overall_accuracy * 100.0);
    println!("  - Reasoning Quality: {:.2}", report.overall_summary.avg_reasoning_quality);
    println!("  - Total Problems: {}", report.overall_summary.total_problems);
    println!("  - Recommendations: {}", report.recommendations.len());

    // Show category breakdown
    println!("\n  📈 Category Performance:");
    for (category, performance) in &report.overall_summary.category_performance {
        println!("    {:?}: {:.1}% accuracy, {:.2} quality", 
                 category, 
    ...
    println!("\n  📝 Markdown report generated ({} characters)", markdown_content.len());
...
fn demo_performance_benchmarking() -> Result<()> {
    ...
    for benchmark in evaluator.get_benchmarks() {
        println!("  - {}: {} problems", benchmark.name, benchmark.problems.len());
    }
    ...
    println!("  - Total time: {:.2}s", performance_metrics.total_time.as_secs_f32());
    println!("  - Average time per problem: {:.2}s", performance_metrics.avg_time_per_problem.as_secs_f32());
    println!("  - Min time: {:.2}s", performance_metrics.min_time.as_secs_f32());
    println!("  - Max time: {:.2}s", performance_metrics.max_time.as_secs_f32());
    println!("  - Memory usage: {:.1} MB", performance_metrics.memory_usage_mb);
    println!("  - Tokens per second: {:.1}", performance_metrics.tokens_per_second);
    println!("  - Reasoning overhead: {:.1}%", performance_metrics.reasoning_overhead * 100.0);
...
fn create_mock_benchmark_result(
    ...
    use ds_r1_rs::utils::evaluation::{BenchmarkResults, ReasoningMetrics, PerformanceMetrics, ProblemResult};
    use std::time::Duration;

    let detailed_results: Vec<ProblemResult> = (0..total).map(|i| {
        ProblemResult {
            ...
            generated_answer: if i < correct { "Correct answer" } else { "Wrong answer" }.to_string(),
